...
framebg = "#EDEDED"
framefg ="#06283D"
path="C:\\Users\\Japjot Singh\\Desktop\\Python\\Projects\\Student_Registration_System\\student-zip\\"
...
def showimage():
  global filename
  global img
  filename=filedialog.askopenfilename(initialdir=os.getcwd(),title="Slect image file",filetype=(("JPG File","*.jpg"),("PNG File","*.png"),("All Files","*.txt")))
...
def Clear():
  ...
  img1 = PhotoImage(file=f"{path}Images/upload photo.png")
...
def Save():
  ...
  try:
      img.save(f"{path}Student Images\\{R1}.jpg")
  except:
...
def search():
  ...
  for row in sheet.rows:
    if row[0].value == int(text):
      ...
      img = (Image.open(f"{path}Student Images\\{x1}.jpg"))
      resized_image = img.resize((190,190))
      photo2 = ImageTk.PhotoImage(resized_image)
      lb1.config(image=photo2)
      lb1.image=photo2
...
Label(root , text="By Japjot Singh Nanda",width=10,height=3,bg="#f0687c",anchor="e").pack(side=TOP,fill=X)
Label(root , text="STUDENT REGISTRATION",width=10,height=2,bg="#c36464",fg="#fff",font="arial 20 bold").pack(side=TOP,fill=X)

# search box to update 
Search = StringVar()
Entry(root,textvariable=Search,width=15,bd=2,font="arial 20").place(x=820,y=70)
imageicon3 = PhotoImage(file=f"{path}Images\\search.png")
Srch=Button(root,text="Search",compound=LEFT,image=imageicon3,width=123,bg="#68ddfa",font="arial 13 bold",command=search)
Srch.place(x=1060,y=66)

imageicon4 = PhotoImage(file=f"{path}Images\\Layer 4.png")
...
date_entry = Entry(root , textvariable=Date,width=15,font="ariual 10")
...
img =PhotoImage(file=f"{path}Images\\upload photo.png")
